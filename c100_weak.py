...
AUX_DIM = 128 # x% reconst error (sklearn truncated SVD)
...
class Cifar100Data(object):
    ...
    def __init__(self, data_dir):
        ...
        # read train data
        images = load(os.path.join(data_dir, 'train'))
        self.images = images # np array
        print('images (numpy): {}'.format(self.images.shape))

        # Read inception logits
        self.feats = np.load('/home/minje/dev/dataset/cifar/cifar100_inception_pool_3_reduced128.npy')
...
class CifarGanModel(object):
    ...
    def generator(self, z, is_training):
        # Standard network
        # l = dense(z, BASE_HEIGHT*BASE_WIDTH*BASE_DIM*16, name='fc_in')
        ...
        l = tf.nn.relu(batch_norm(l, is_training))
        r = l
        img = conv(l, 3, 3, 1, name='conv_out')
        img = tf.nn.tanh(img, name='gen_out')

        r = tf.nn.relu(batch_norm(conv(r, BASE_DIM*8, 7, 4, name='r0'), is_training))        
        r = tf.reshape(r, [-1, np.prod(r.shape.as_list()[1:])])
        r = tf.nn.relu(batch_norm(dense(r, BASE_DIM*8, l2_scale=0.0, name='r1'), is_training))
        ...
        #r = tf.layers.dropout(r, 0.5, training=is_training)
        r = dense(r, AUX_DIM, l2_scale=0.0, name='lab_fc')
    ...
    def input(self):
        ...
        self.labels = tf.placeholder(tf.float32, (None, AUX_DIM), name='labels') # soft label (=inception feat processed)
