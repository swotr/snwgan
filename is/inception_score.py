...
def get_cifar10_batches(path):
    ...
    def load(file):
        ...
        with open(file, 'rb') as fo:
            ...
            images = images.reshape([-1, 3, image_height, image_width])
            return images, labels

    # read 5 training batches
    images, labels = load(os.path.join(path, 'data_batch_1'))
    for i in range(2, 6):
        _images, _labels = load(os.path.join(path, 'data_batch_{}'.format(i)))
        images = np.vstack((images, _images))
        labels.extend(_labels)
    return images

def get_from_files(path):
    import cv2
...
if __name__ == '__main__':
    #images = get_cifar10_batches('/home/minje/dev/dataset/cifar/cifar-10-batches-py/')
    images = get_from_files('/home/minje/dev/dataset/cifar/cifar-10-fake/')
    mean, std = get_inception_score(images, 1)
    print('IS: ', mean, std)
