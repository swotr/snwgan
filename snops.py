...
def spectral_normed_weight(w, num_iters=1, update_collection=None):
    ...
    w = tf.reshape(w, [-1, w_shape[-1]])  # [-1, output_channel]
    u = tf.get_variable('u', [1, w_shape[-1]],
                        initializer=tf.truncated_normal_initializer(), trainable=False)
    _u = u
    _v = None
    for _ in range(num_iters):
        _v = tf.nn.l2_normalize(tf.matmul(_u, w, transpose_b=True))
        _u = tf.nn.l2_normalize(tf.matmul(_v, w))
    
    sigma = tf.squeeze(tf.matmul(tf.matmul(_v, w), _u, transpose_b=True))
